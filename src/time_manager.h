...
inline void Optimum(SearchInfo* info, int time, int inc) {
...
[[nodiscard]] inline bool StopEarly(const SearchInfo* info) {
...
[[nodiscard]] inline bool NodesOver(const SearchInfo* info) {
    // check if we used all the nodes/movetime we had or if we used more than our lowerbound of time
    return info->nodeset && info->nodes >= info->nodeslimit;
}

[[nodiscard]] inline bool TimeOver(const SearchInfo* info) {
