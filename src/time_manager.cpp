...
void ScaleTm(const int bestMoveStabilityFactor, const int evalStabilityFactor) {
    ...
    const double bestMoveNodesFraction = static_cast<double>(nodeSpentTable[FromTo(bestmove)]) / static_cast<double>(mainTD.nodes);
...
bool NodesOver() {
    // check if we used all the nodes/movetime we had or if we used more than our lowerbound of time
    return info.nodeset && mainTD.nodes >= info.nodeslimit;
}

bool TimeOver() {
    // check if more than Maxtime passed and we have to stop
    return NodesOver() || ((info.timeset || info.movetimeset)
                               && ((mainTD.nodes & 1023) == 1023)
